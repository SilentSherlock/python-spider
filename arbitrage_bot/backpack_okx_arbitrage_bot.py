...
# === 用户参数设置 ===
OKX_SYMBOL = "KAVA-USDT-SWAP"  # OKX 的永续合约标识（示例）
...
# === 工具函数 ===
def get_okx_funding_rate(symbol):
...
def get_backpack_price(pair):
    url = f"https://api.backpack.exchange/api/v1/market/ticker?symbol={pair}"
    r = requests.get(url).json()
...
def execute_okx_order(symbol, side, qty):
    print(f"[模拟下单] OKX {side} {qty} {symbol}")
    # TODO: 接入 OKX 下单接口
...
def arbitrage_loop():
    while True:
        try:
            okx_rate, next_time = get_okx_funding_rate(SYMBOL)
            backpack_price = get_backpack_price(BACKPACK_SYMBOL)

            print(f"[{datetime.utcnow().isoformat()}] OKX 资金费率: {okx_rate:.4%}, Backpack 价格: {backpack_price}, 下次结算: {next_time}")
            ...
            if abs(okx_rate) >= THRESHOLD_DIFF and within_funding_window(next_time, SETTLEMENT_WINDOW_MIN):
                ...
                if direction == "LONG":
                    execute_okx_order(SYMBOL, "buy", qty)
                    execute_backpack_order(BACKPACK_SYMBOL, "sell", qty)
                else:
                    execute_okx_order(SYMBOL, "sell", qty)
